...
# ============================================================
# ENDPOINTS: usuarios (lista pública para admin panel)
# ============================================================
...
def get_users():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT id_usuario AS Id, Nombre, Email, Tipo AS tipo_usuario
            FROM usuarios
            ORDER BY Nombre
        """)
        rows = cur.fetchall()
        cur.close()
        conn.close()
        users = [{"id": r[0], "nombre": r[1], "email": r[2], "tipo": r[3]} for r in rows]
...
# ============================================================
# ENDPOINT: GENERAR IMAGEN CON IA (OpenAI DALL·E)
# ============================================================

@app.post('/api/generate-image')
async def generate_image(payload: dict):
    """
    Generar imagen con OpenAI DALL·E (gpt-image-1).
    Migrado desde generate-image.php.
    ...
    if not prompt_raw:
        raise HTTPException(status_code=400, detail={"success": False, "error": "El prompt es requerido"})

    api_key = os.getenv('OPENAI_API_KEY', '')
    if not api_key:
        raise HTTPException(status_code=500, detail={"success": False, "error": "OPENAI_API_KEY no configurada en el servidor"})
    ...
    try:
        ...
        if resp.status_code != 200:
            raise HTTPException(status_code=502, detail={"success": False, "error": f"Error OpenAI HTTP {resp.status_code}"})

        result = resp.json()
        b64 = result.get('data', [{}])[0].get('b64_json')
        if not b64:
            raise HTTPException(status_code=500, detail={"success": False, "error": "No se pudo obtener la imagen de OpenAI"})

        # Guardar en disco
        img_data = base64.b64decode(b64)
        filename = f"img_{int(datetime.now().timestamp())}_{secrets.token_hex(4)}.png"
        save_path = IMAGENES_IA_DIR / filename
...
# ============================================================
# ENDPOINT: CREAR PAGO CON MERCADOPAGO
# ============================================================
...
def create_payment(payload: dict):
    ...
    Migrado desde create-payment.php.
    ...
    try:
        ...
        if not access_token:
            raise HTTPException(status_code=500, detail={"success": False, "error": "MERCADOPAGO_ACCESS_TOKEN no configurado en el servidor"})
        ...
        preference_data = {
            ...
            "payer": {
                "email": payload.get('email', 'cliente@aiprint.com')
            },
            "back_urls": {
...
# ============================================================
# ENDPOINT: CONFIRMAR NOTIFICACIÓN DE PAGO (back_url de MercadoPago)
# ============================================================

@app.post('/api/save-payment')
def save_payment(payload: dict):
    """
    Registrar notificación de pago recibida desde la back_url de MercadoPago.
    Migrado desde save-payment.php (que solo escribía en un .txt).
    En producción esto debería manejarse vía webhook IPN/webhook de MP.
    """
    try:
        payment_id = payload.get('payment_id')
        status = payload.get('status')
        print(f'[save-payment] payment_id={payment_id} status={status}')
        ...
        if payment_id and status == 'approved':
            ...
            cur.execute("""
                UPDATE Pedidos
                SET estado_pago = 'pagado', referencia_externa = ?
                WHERE referencia_externa = ? OR referencia_externa IS NULL
        ...
        print(f'Error en save_payment: {e}')
        raise HTTPException(status_code=500, detail={"success": False, "error": str(e)})
...
# ============================================================
# ENDPOINT: MIS PEDIDOS - Cliente puede ver su historial de compras
# ============================================================

@app.get('/api/mis-pedidos/{id_usuario}')
def get_mis_pedidos(id_usuario: int):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM usuarios WHERE id_usuario = ?", (id_usuario,))
        if cur.fetchone()[0] == 0:
            raise HTTPException(status_code=404, detail={"success": False, "error": "Usuario no encontrado"})

        # Obtener cabecera de pedidos del usuario
        cur.execute("""
            SELECT
                p.id_pedido, p.numero_orden, p.fecha_pedido,
                p.estado, p.estado_pago, p.total,
                p.direccion_envio, p.ciudad
            FROM Pedidos p
            WHERE p.id_usuario = ?
        ...
        for fila in filas_pedidos:
            ...
            # Obtener items con producto, variante y thumbnail
            cur.execute("""
                SELECT
                ...
                WHERE pi.id_pedido = ?
                GROUP BY pi.cantidad, pi.precio_unitario,
                         prod.nombre, ad.ruta_thumbnail
            """, (id_pedido,))
        ...
        print(f'Error en get_mis_pedidos: {str(e)}')
        raise HTTPException(status_code=500, detail={"success": False, "error": str(e)})
