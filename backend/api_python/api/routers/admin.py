    ...
    try:
        ...
        for idx, row in enumerate(cur.fetchall()):
            ...
            cur.execute("""
                ...
                GROUP BY pi.cantidad, pi.precio_unitario, prod.nombre, pi.archivo_diseno
            """, (id_pedido,))

            items = [{"cantidad": r[0], "precio_unitario": float(r[1]), "producto_nombre": r[2], "color": r[3] or "-", "talle": r[4] or "-", "archivo_diseno": r[5]} for r in cur.fetchall()]
            ...
            # Obtener ruta de imagen del diseño
            imagen_diseno = None
            if detalle.get("archivo_diseno"):
                archivo_id = detalle["archivo_diseno"]
                if isinstance(archivo_id, int):
                    cur.execute("SELECT ruta_archivo FROM archivos_diseno WHERE id_archivo = %s", (archivo_id,))
                    r_img = cur.fetchone()
                    if r_img:
                        imagen_diseno = r_img[0]
