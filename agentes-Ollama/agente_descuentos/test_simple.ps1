...
Write-Host ""
Write-Host "[2/4] Listando cupones actuales..." -ForegroundColor Yellow
try {
    $cupones = Invoke-RestMethod -Uri "http://localhost:5003/api/cupones"
    Write-Host "  OK - Cupones encontrados: $($cupones.total)" -ForegroundColor Green
    foreach ($cupon in $cupones.cupones) {
        Write-Host "    - $($cupon.codigo): $($cupon.descuento_porcentaje)% OFF" -ForegroundColor Gray
    }
}
catch {
    Write-Host "  ERROR - Error listando cupones" -ForegroundColor Red
}

# Test 3: Obtener estadísticas
Write-Host ""
Write-Host "[3/4] Obteniendo estadisticas de ventas..." -ForegroundColor Yellow
try {
...
# Test 4: Verificar Ollama
Write-Host ""
Write-Host "[4/4] Verificando Ollama para propuestas IA..." -ForegroundColor Yellow
try {
    ...
    Write-Host "  INFO - Ollama no disponible (opcional)" -ForegroundColor Yellow
